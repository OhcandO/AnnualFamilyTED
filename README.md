...
### 통합 소개 페이지 (`docs/index.html`)

GitHub Pages의 메인 진입점. 넷플릭스풍 다크 히어로에서 시작해 스크롤할수록
새벽처럼 밝아지는 단일 페이지로, 의존성 없는 순수 HTML/CSS/JS입니다.

- Hero (다크) → 초대의 글 → TED 소개 (TED가 처음인 가족을 위한 설명)
- 하루의 흐름: 점심 식사 → 파티룸 이동 → 본행사 → 다과 → 기념사진·종료
- 장소(TBA·파티룸 시설 일러스트) → 드레스 코드(주황 포인트 아이템 예시) → 클로징
- 노안 배려: 큰 글자·고대비, 모바일(갤럭시/아이폰) 우선 레이아웃
- 푸터에서 아래 두 에디션으로 이동 가능

### 기존 에디션 (`docs/light.html` · `docs/dark.html`)
